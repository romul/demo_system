defmodule ExampleSystemWeb.Top.Dashboard do
  ...
  @impl Phoenix.LiveView
  def handle_event("process_trace", %{"pid" => pid_str}, socket),
    do: {:noreply, assign(socket, :output, process_trace(:erlang.list_to_pid(~c"<#{pid_str}>")))}
  ...
  defp inspect_args(args),
    do:
      args
      |> inspect(limit: 5, pretty: true, width: 100)
      |> String.replace(~r/^\[/, "")
      |> String.replace(~r/\]$/, "")
  ...
  defp format_stacktrace_entry({mod, fun, arity, location}),
    do: "#{inspect(mod)}.#{fun}/#{arity} #{location(location)}"
